...
| **libvirt XML** (`configs/libvirt/windows-eng.local.xml`) | Host backup (not Git: it's gitignored) | Every edit | Holds your UUID, paths and device IDs. The generic template is in Git; `virsh dumpxml --inactive windows-eng` also recovers the live definition. |
