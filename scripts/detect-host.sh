...
# Suggested cmdline: hugepages = the guest's <memory> in GiB (local copy first).
guest_xml="$(dirname "$0")/../configs/libvirt/windows-eng.local.xml"
[[ -f "$guest_xml" ]] || guest_xml="$(dirname "$0")/../configs/libvirt/windows-eng.xml"
